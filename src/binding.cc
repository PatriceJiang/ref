...
NAN_METHOD(ReadPointer) {
  ...
  int64_t offset = GetInt64(info[1]);
  char *ptr = Buffer::Data(buf.As<Object>()) + offset;
  size_t size = info[2]->Uint32Value(Nan::GetCurrentContext()).FromJust();
...
NAN_METHOD(ReinterpretBuffer) {
  ...
  size_t size = info[1]->Uint32Value(Nan::GetCurrentContext()).FromJust();
...
NAN_METHOD(ReinterpretBufferUntilZeros) {
  ...
  uint32_t numZeros = info[1]->Uint32Value(Nan::GetCurrentContext()).FromJust();
  uint32_t i = 0;
  size_t size = 0;
  bool end = false;
